    ...
    switch (state) {
      ...
      case S_CS:
        ...
        while (cs_elapsed_time < CS_DURATION) {
          digitalWrite(CS_OUT, (cs_elapsed_time < CS_INIT) || enable_cs);
          ...
          if ((abs(steps_fwd) + abs(steps_bwd)) < THRESHOLD_STEPS) {
            if (cs_elapsed_time > CS_INIT) {
              enable_cs = false;
              enable_us = false;
            }
          }
